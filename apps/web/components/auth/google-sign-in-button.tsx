...
export function GoogleSignInButton({ callbackURL, label, className, disabled, onError }: Props) {
  ...
  return (
    <button
      ...
      aria-busy={busy}
      className={cn(
        'border-border bg-card text-foreground focus-visible:ring-ring focus-visible:ring-offset-background flex min-h-11 w-full items-center justify-center gap-3 rounded-full border px-4 text-sm font-medium transition-opacity hover:opacity-90 focus-visible:outline-hidden focus-visible:ring-2 focus-visible:ring-offset-2 disabled:opacity-40',
        className,
      )}
      onClick={() => {
        setBusy(true);
