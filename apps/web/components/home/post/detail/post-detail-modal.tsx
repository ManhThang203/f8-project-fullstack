...
import { usePostComments } from '@/hooks/queries/posts';
...
export function PostDetailModal({ open, onClose, post, me }: Props) {
  ...
  const { data, fetchNextPage, hasNextPage, isLoading, isFetchingNextPage } = usePostComments(
    post.id,
  );
  ...
  async function handleSubmit() {
    ...
    const finalContent =
      replyingToUsername && !text.startsWith(`@${replyingToUsername}`)
            ...
            <div className="bg-muted/50 min-w-0 flex-1 rounded-2xl p-3">
              ...
              <div className="border-border/50 mt-2 flex items-center justify-between border-t pt-2">
                <div className="flex items-center gap-1">
                  ...
                  <ComposeEmojiPicker onSelect={handleEmojiSelect} size="sm" disabled={busy} />
                </div>
                <button
